...
def find_growth_between(pkg, old_month, new_month):
  with open("%s.json" % pkg, "r") as f:
    ...
    percent = (new_dl / old_dl) * 100
    change = 'grew'
    if (percent < 0):
      change = 'declined'
    percent_str = float("{0:.2f}".format(percent))

    return percent, "{}'s download rate {} by {}% between {} and {}".format(pkg, change, percent_str, old_month, new_month)

if __name__ == '__main__':
  pkg = sys.argv[1]
  old_month = sys.argv[2]
  new_month = sys.argv[3]
  percent, growth_str = find_growth_between(pkg, old_month, new_month)
  print(growth_str)